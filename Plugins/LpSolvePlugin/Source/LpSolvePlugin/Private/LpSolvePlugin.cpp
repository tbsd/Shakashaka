...
void FLpSolvePluginModule::StartupModule()
{
	...
	ExampleLibraryHandle = !LibraryPath.IsEmpty() ? FPlatformProcess::GetDllHandle(*LibraryPath) : nullptr;
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 3"));
	o << "HEHMDA 3" << std::endl;
#else
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 4"));
	o << "HEHMDA 4" << std::endl;
	...
	o << "HEHMDA 5" << std::endl;
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 5"));
	ExampleLibraryHandle = !LibraryPath.IsEmpty() ? FPlatformProcess::GetDllHandle(*LibraryPath) : nullptr;
	o << "HEHMDA 6" << std::endl;
	if (!ExampleLibraryHandle) {
	o << "HEHMDA 7" << std::endl;
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 6"));
	...
	o << "HEHMDA 8" << std::endl;
		ExampleLibraryHandle = !LibraryPath.IsEmpty() ? FPlatformProcess::GetDllHandle(*LibraryPath) : nullptr;
	o << "HEHMDA 9" << std::endl;
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 7"));
	}
	...
	o << "HEHMDA 11" << std::endl;
		...
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 8"));
	}
	else {
	o << "HEHMDA 12" << std::endl;
		UE_LOG(LogTemp, Error, TEXT("lpsolve failed to load!"));
		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 9"));
	}

		UE_LOG(LogTemp, Error, TEXT("HEHMDA a 10"));
